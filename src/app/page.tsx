...
export default function StudentPage() {
  ...
  const qpPathway = useQueryParam("pathway_id");
  const fixedPathwayId = qpPathway ? Number(qpPathway) : null;

  const [student, setStudent] = useState<UserOut | null>(null);
  const [pathways, setPathways] = useState<Pathway[]>([]);
  const [pathwayId, setPathwayId] = useState<number | null>(null);
  ...
  // Load pathways (public list)
  useEffect(() => {
    apiGet<Pathway[]>("/pathways?active=true")
      .then((list) => {
        setPathways(list);

        // Initial selection logic:
        if (fixedPathwayId && Number.isFinite(fixedPathwayId)) {
          setPathwayId(fixedPathwayId);
        } else if (list.length > 0) {
          setPathwayId(list[0].pathway_id);
        } else {
          setPathwayId(null);
        }
      })
      .catch((err: unknown) => {
        ...
        addToast(msg, "error");
        // no pathways -> ensure state reflects that
        setPathwayId(null);
      });
  }, [fixedPathwayId, addToast]);

  // Optional SSM directory (fallback to “SSM {id}” on failure)
  useEffect(() => {
    apiGet<SSMInfo[]>("/ssms")
      .then((ssms) => {
        const map: Record<number, SSMInfo> = {};
        ssms.forEach((s) => (map[s.id] = s));
        setSsmInfo(map);
      })
      .catch(() => {
        /* ignore */
  ...
  // Load availability for selected pathway
  useEffect(() => {
    if (!pathwayId) return;
  ...
  // Build visible slots (2 weeks ahead, 30-min blocks) from /availability/{id}
  const slots = useMemo<SlotView[]>(() => {
    ...
    for (const avail of data.availabilities) {
      for (let w = 0; w < 2; w++) {
        ...
        const deltaDays = ((avail.day_of_week - todayDow + 7) % 7) + w * 7;
  ...
  async function book() {
    if (!student || !discoUserId || !selectedSlot || !pathwayId) return;
  ...
  const showPathwaySelector =
    !fixedPathwayId && pathways.length > 1 && pathwayId !== null;
                  ...
                  <div className="flex gap-3 flex-wrap">
                    {pathways.map((p) => (
                      <button
                        ...
                        title={p.description || ""}
                      >
                        {p.name ? p.name : `Pathway ${p.pathway_id}`}
                      </button>
                    ))}
                  </div>
                </div>
